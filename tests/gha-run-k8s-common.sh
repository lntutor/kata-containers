	...
	case "${KUBERNETES}" in
		...
		vanilla)
			if [[ "${SNAPSHOTTER:-}" == "erofs" ]]; then
				# fsverity comes along with erofs-utils here
				# because, unlike the docker and nerdctl jobs,
				# these do enable fs-verity on the layer blobs.
				install_erofs_utils fsverity
