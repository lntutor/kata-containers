...
# Installs erofs-utils (plus any extra package passed as an argument) and loads
# the erofs module.
#
# mkfs.erofs has to be >= 1.8 for the flags containerd's erofs differ passes
# (-T0, --mkfs-time, --sort), and Ubuntu 24.04 still ships 1.7.1, so take it
# from Ubuntu 25.10 (Questing Quokka), pinned low enough that nothing else gets
# pulled in from that release.
function install_erofs_utils() {
	local -a packages=(erofs-utils "$@")
	...
	APTPIN
	sudo apt-get update
	sudo apt-get -y install --no-install-recommends -t questing "${packages[@]}"
